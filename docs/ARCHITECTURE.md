...
## 2. Stack Tecnológico
- **Frontend (Web):** React 19, Vite, TailwindCSS, Socket.io-client.
- **Frontend (Mobile):** React Native (Expo SDK 54), Socket.io-client.
- **Backend:** Node.js, Express 5, TypeScript estricto.
- **Base de Datos:** PostgreSQL (vía Supabase), Prisma ORM 6.
...
- **Testing:** Jest + Supertest (meta de calidad: suite proyectada de 120+ pruebas unitarias y de integración).
...
- El Panel Administrativo escucha eventos `admin:event` para actualizar sus estadísticas y el registro de usuarios en vivo.
...
- Si un administrador elimina una cuenta que ya ha participado en consultas médicas, el sistema reemplaza el nombre por "Usuario Eliminado" y el correo por un formato encriptado (`anon_${uuid}@deleted.vetconnect.internal`).
...
- **Manejo de Errores Global:** Middleware asíncrono unificado (`asyncHandler`) que centraliza excepciones y devuelve respuestas HTTP estándar RFC 7807.
- **Rotación de Sesiones:** Cuando un usuario cambia su contraseña o es eliminado, se incrementa un `tokenVersion` en su base de datos. Cualquier JSON Web Token (Access o Refresh) emitido previamente queda invalidado instantáneamente.

## 6. Criterios de Aceptación y Metas de Calidad Arquitectónica (Objetivo 100/100)
- **Seguridad:** A+ (BCrypt 12 rounds, JWT estricto con HS256, rotación de sesiones, inyecciones mitigadas por Prisma).
- **Rendimiento:** A+ (Caché Redis, queries indexadas, payload JSON mínimo).
- **Mantenibilidad:** A+ (Tipado estricto al 100%, Cero warnings de TypeScript al compilar, meta proyectada de 120+ Tests automatizados con >80% de cobertura).
