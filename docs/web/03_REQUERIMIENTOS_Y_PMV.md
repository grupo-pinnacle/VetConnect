...
| **RF-WEB-03** | Triage Telemédico Guiado | El sistema debe proporcionar un cuestionario secuencial reactivo de síntomas que clasifique la urgencia en tres niveles semánticos: `VERDE` (Leve), `AMARILLO` (Moderada) y `ROJO` (Vital). **El identificador de prioridad se almacena en español** (`'ROJO' | 'AMARILLO' | 'VERDE'`) dentro del campo `notes` de la consulta, antepuesto como `[Prioridad: ROJO] descripción`. El sistema de badges del dashboard veterinario detecta la prioridad buscando estas cadenas exactas en el campo `notes`. | `CLIENT` |
...
2. **Registro e Inicio de Sesión:** Login universal por email y contraseña; registro con selector de rol (Tutor / Veterinario) y validaciones Zod en tiempo real conforme al alcance v2.0 (recuperación de contraseña diferida a v2.1+).
...
| **`consultations`**| `POST` | `/api/consultations` | `pages/DashboardClient.tsx` (Triage) | Body: `{ petId, notes }` (La prioridad elegida `ROJO`\|`AMARILLO`\|`VERDE` se antepone en notes como `[Prioridad: ${priority}] ${notes}`) → `{ success: true, data: Consultation }` | `modules/consultations/consultations.schemas.ts` |
...
| **`consultations`**| `PATCH` | `/api/consultations/:id/assign` | `pages/DashboardVet.tsx` | Param: `id` (Consultation UUID) → `{ success: true, data: Consultation }`. Requiere rol `VET` con `vetStatus=APPROVED`. | `modules/consultations/consultations.schemas.ts` |
| **`consultations`**| `PATCH` | `/api/consultations/:id/cancel` | `pages/DashboardClient.tsx`, `pages/ConsultationRoom.tsx` | Param: `id` → Transición a estado `CANCELLED`. Disponible para CLIENT (tutor) y ADMIN. | `modules/consultations/consultations.schemas.ts` |
...
| **`consultations`**| `POST` | `/api/consultations/:id/review` | `pages/ConsultationRoom.tsx` (overlay `<ReviewModal>` cuando `phase === 'completed'`) | Body: `{ rating: number (1-5), comment?: string }` → Calificación médica. Solo CLIENT asignado. ADR-023. | `modules/consultations/consultations.schemas.ts` |
...
| **`prescriptions`**| `POST` | `/api/consultations/:id/prescriptions` | `pages/DashboardVet.tsx (y pages/ConsultationRoom.tsx vía <PrescriptionModal> en Fase 3 CDD)` | Body: `{ medication, dosage, frequency, durationDays, indications }` (`indications` obligatorio min 5 chars) → `{ success: true, data: Prescription }` | `modules/prescriptions/prescriptions.schemas.ts` |
...
| **`media`** | `POST` | `/api/media` | `pages/ConsultationRoom.tsx` | `multipart/form-data` (archivo imagen, validación Magic Bytes, cuota diaria) → `{ success: true, data: MediaFile }` donde `MediaFile` contiene `{ id, ownerId, consultationId, fileName, fileSize, mimeType, localPath, s3Key, createdAt, updatedAt, deletedAt }`. ⚠️ **Nota de Integración Crítica: `MediaFile` NO tiene campo `url`.** El frontend construye la URL de acceso autenticado manualmente como `` `/api/media/${data.id}` `` (ver `ConsultationRoom.tsx`). | `middlewares/media.middleware.ts (Multer + Magic Bytes binarios)` |
