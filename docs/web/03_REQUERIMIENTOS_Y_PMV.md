...
| **`pets`** | `POST` | `/api/pets` | `pages/DashboardClient.tsx` | Body: `{ name, species, breed, weightKg?, sex?, microchip?, allergies?, chronicConditions? }` → `{ success: true, data: Pet }` | `modules/pets/pets.schemas.ts` |
...
| **`consultations`**| `GET` | `/api/consultations/mine` | `pages/DashboardClient.tsx`, `pages/DashboardVet.tsx` | Listar consultas del usuario autenticado → `{ success: true, data: Consultation[] }` | `modules/consultations/consultations.schemas.ts` |
| **`consultations`**| `POST` | `/api/consultations` | `pages/DashboardClient.tsx` (Triage) | Body: `{ petId, notes }` (Convención UI Triage: `[Prioridad: ${priority}] ${notes}`) → `{ success: true, data: Consultation }` | `modules/consultations/consultations.schemas.ts` |
...
| **`calls`** | `POST` | `/api/calls/:consultationId/token` | `pages/ConsultationRoom.tsx` | Param: `consultationId` (UUID v4) → `{ success: true, data: { token: string, wsUrl: string } }` (Zero PII) | `modules/calls/calls.schemas.ts` |
...
| **`prescriptions`**| `POST` | `/api/consultations/:id/prescriptions` | `pages/ConsultationRoom.tsx` | Body: `{ medication, dosage, frequency, durationDays, indications }` (`indications` obligatorio min 5) → `{ success: true, data: Prescription }` | `modules/prescriptions/prescriptions.schemas.ts` |
...
| **`media`** | `POST` | `/api/media` | `pages/ConsultationRoom.tsx` | `multipart/form-data` (Límite 10 MB/archivo, cuota diaria 50 MB/día por usuario `DailyUploadCounter`, Magic Bytes JPEG/PNG/WEBP/PDF. Errores: `413 FILE_TOO_LARGE`, `429 UPLOAD_QUOTA_EXCEEDED`) → `{ success: true, data: MediaFile }` | `modules/media/media.schemas.ts` |
