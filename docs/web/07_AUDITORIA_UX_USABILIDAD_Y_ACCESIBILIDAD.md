...
| Criterio Evaluado | Estado | Hallazgo o Riesgo Detectado | Propuesta de Solución / Estado Real |
|---|---|---|---|
| **Protección contra Errores** | ⏳ **Pendiente** | La acción de dar de baja a una mascota podría ejecutarse accidentalmente por un tutor nervioso. | **Implementación pendiente:** Agregar un modal de confirmación de dos pasos en `DashboardClient.tsx` antes de llamar a `DELETE /api/pets/:id`. El modal debe advertir que la historia clínica quedará archivada mediante *soft-delete* (`deletedAt`). Se implementará como `web/src/components/ui/ConfirmModal.tsx` con HTML5 nativo (`role="dialog"`, `aria-modal="true"`), sin Radix UI. |
...
| **Visibilidad del Estado** | ⏳ **Pendiente** | En conexiones lentas, el tutor no sabía si el veterinario estaba por ingresar a la sala. | **Implementación pendiente:** La pantalla de `phase === 'waiting'` en `ConsultationRoom.tsx` (implementada en la corrección del estado de espera) ya muestra el spinner de pulso básico. El banner avanzado con nombre del médico asignado y ETA (`"El Dr. Mendoza está revisando la ficha de Milo..."`) se implementará cuando el backend incluya el campo `vet` en el polling de `GET /api/consultations/:id` con estado `ACTIVE`. |
| **Capacidad de Aprendizaje** | ⏳ **Pendiente** | El ingreso del microchip ISO generaba dudas en tutores que no recordaban si su mascota lo tenía. | **Implementación pendiente:** Agregar en el formulario de alta de mascotas en `DashboardClient.tsx` un checkbox: *"No posee microchip o no lo recuerdo ahora"* que desactive el campo `microchip`. Control accesible con `aria-describedby` explicando el ISO-11784/11785. |
| **Navegación por Teclado** | ✅ Cumple | El visor Lightbox de macro-fotografías clínicas no permitía cerrar la imagen con la tecla `Escape`. | Se integró un listener global de teclado para la tecla `Escape` (`keydown → setZoomImage(null)`) y foco atrapado (*Focus Trap*) dentro del modal en `ConsultationRoom.tsx`. |
| **Prevención de Errores (Recetas)** | ⏳ **Pendiente** | Un error de tipeo en la dosis de un fármaco podría comprometer la salud del paciente. | **Implementación pendiente:** El formulario actual en `DashboardVet.tsx` tiene campos de texto libre. Implementar campos de posología con lista predefinida de medicamentos comunes y una vista previa obligatoria (`<PrescriptionDoc />`) antes de confirmar el envío a `POST /api/consultations/:id/prescriptions`. Parte de CDD Fase 3 (`PrescriptionModal.tsx`). |
