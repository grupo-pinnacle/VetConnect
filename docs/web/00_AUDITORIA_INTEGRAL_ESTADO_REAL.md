...
| **Videoconsulta WebRTC HD** | ⚠️ **90% Implementado (Gaps de Sala de Espera y Cierre)** | En [`ConsultationRoom.tsx`](../../web/src/pages/ConsultationRoom.tsx) y [`CallRoom.tsx`](../../web/src/components/call/CallRoom.tsx). Implementa LiveKit Cloud SFU sin duplicar audio renderers (cero eco acústico), soporte dinámico de `wsUrl`, modal de pre-chequeo de cámara/micrófono ([`PreJoinModal.tsx`](../../web/src/components/call/PreJoinModal.tsx)) y handshake `page:ready` para WebView móvil. **Gaps conocidos:** (1) La máquina de estados `WAITING → ACTIVE` (polling 5s + UI de sala de espera) NO está implementada — actualmente `ConsultationRoom.tsx` solicita el token LiveKit al montar sin verificar `status`. (2) `PrescriptionModal` NO está montado en `ConsultationRoom.tsx` (solo en `DashboardVet.tsx`). (3) `ReviewModal` es un componente nuevo a crear. |
